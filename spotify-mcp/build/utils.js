...
export function loadSpotifyConfig() {
    if (!fs.existsSync(CONFIG_FILE)) {
        throw new Error(`Spotify configuration file not found at ${CONFIG_FILE}. Please create one with clientId, clientSecret, and redirectUri.`);
    }
    try {
        ...
        throw new Error(`Failed to parse Spotify configuration: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function authorizeSpotify() {
    ...
    if (redirectUri.hostname !== 'localhost' &&
        ...
        console.error('Please update your spotify-config.json with a localhost redirect URI');
    ...
    const authPromise = new Promise((resolve, reject) => {
        ...
        const server = http.createServer(async (req, res) => {
            ...
            if (reqUrl.pathname === callbackPath) {
                ...
                if (error) {
                    ...
                    res.end('<html><body><h1>Authentication Failed</h1><p>Please close this window and try again.</p></body></html>');
                    server.close();
                ...
                if (returnedState !== state) {
                    ...
                    res.end('<html><body><h1>Authentication Failed</h1><p>State verification failed. Please close this window and try again.</p></body></html>');
                    server.close();
                ...
                if (!code) {
                    ...
                    res.end('<html><body><h1>Authentication Failed</h1><p>No authorization code received. Please close this window and try again.</p></body></html>');
                    server.close();
                ...
                try {
                    ...
                    res.end('<html><body><h1>Authentication Successful!</h1><p>You can now close this window and return to the application.</p></body></html>');
                    ...
                    res.end('<html><body><h1>Authentication Failed</h1><p>Failed to exchange authorization code for tokens. Please close this window and try again.</p></body></html>');
                    server.close();
                    reject(error);
                }
            }
        ...
        server.listen(Number.parseInt(port), '127.0.0.1', () => {
            console.log(`Listening for Spotify authentication callback on port ${port}`);
            console.log('Opening browser for authorization...');
            open(authorizationUrl).catch((_error) => {
                console.log('Failed to open browser automatically. Please visit this URL to authorize:');
